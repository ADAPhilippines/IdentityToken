...
public class CardanoHelper
{
  ...
  public static ulong CalculateTotalFromOutputs(IEnumerable<CardanoTxOutput> outputs)
  {
    ulong total = 0;
    outputs.ToList().ForEach((x) =>
    {
      if (x != null && x.Amount != null)
      {
        x.Amount.ToList().ForEach((y) =>
        {
          total += y.Quantity;
        });
      }
